...
public class WorldLoader {
	...
	private ArrayDeque<Integer> chunkIdList;
	...
	public void onChunkModified(Chunk chunk) {
		...
		updateChunkSlow(chunk.x+1, chunk.z+1);
		updateChunkSlow(chunk.x+1, chunk.z);
		updateChunkSlow(chunk.x+1, chunk.z-1);
		updateChunkSlow(chunk.x, chunk.z+1);
		updateChunkSlow(chunk.x, chunk.z-1);
		updateChunkSlow(chunk.x-1, chunk.z+1);
		updateChunkSlow(chunk.x-1, chunk.z);
		updateChunkSlow(chunk.x-1, chunk.z-1);
	...
	public void updateWorld(double posX, double posY, double posZ, Shader shader) {
		...
		synchronized (chunksModifiedFast) {
			while (!chunksModifiedFast.isEmpty()) {
				...
				int chunkX = chunk.x - playerChunkX + renderDistance-1;
				int chunkZ = chunk.z - playerChunkZ + renderDistance-1;
			...
			for (int z = 0; z < 16; z++) {
				for (int x = 0; x < 16; x++) {
					...
					case 113: //nether brick fence
						newId = Maps.getFence(oldId, chunk.getWorld(), new BlockPos(chunk.x*16 + x, height*16 + y, chunk.z*16 + z));
						break;
					case 101: //iron bars
					case 102: //glass pane
						newId = Maps.getPane(oldId, chunk.getWorld(), new BlockPos(chunk.x*16 + x, height*16 + y, chunk.z*16 + z));
						break;
					case 139: //cobblestone wall
						newId = Maps.getWall(oldId, chunk.getWorld(), new BlockPos(chunk.x*16 + x, height*16 + y, chunk.z*16 + z));
						break;
					...
					data[y*16*16*4 + z*16*4 + x*4 + 2] = storage[height].getBlockLight().get(x, y, z);
